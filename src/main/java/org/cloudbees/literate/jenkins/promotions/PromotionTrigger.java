    ...
    public static class DescriptorImpl extends TriggerDescriptor {
        ...
        public FormValidation doCheckJobName(@AncestorInPath Item project, @QueryParameter String value ) {
            ...
            if (StringUtils.isNotBlank(value)) {
                LiterateMultibranchProject p = Jenkins.getInstance().getItem(value,project,LiterateMultibranchProject.class);
                if(p==null)
                    return FormValidation.error(hudson.tasks.Messages.BuildTrigger_NoSuchProject(value,
                            LiterateMultibranchProject.findNearest(value, project.getParent()).getRelativeNameFrom(project)));
         ...
         */
        public ListBoxModel doFillBranchNameItems(@AncestorInPath Item defaultJob, @QueryParameter("jobName") String jobName) {
         ...
         * Fills in the available promotion processes.
         */
        public ListBoxModel doFillProcessItems(@AncestorInPath Item defaultJob, @QueryParameter("branchName") String branchName, @QueryParameter("jobName") String jobName) {
            defaultJob.checkPermission(Item.CONFIGURE);
